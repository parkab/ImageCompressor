public class Quantization {
	static int[][] quant_table_luma = {{16,11,10,16,24,40,51,61},
	...
	static int[][] quant_table_chroma = {{17,18,24,47,99,99,99,99},
	...
	public static int[][] quantizeLuma(int[][] input) {
	...
	public static int[][] decodeQuantizeLuma(int[][] input) {
		int[][] quantizedArray = new int[input.length][input[0].length];
		for(int i = 0; i < input.length; i++) {
			for(int j = 0; j < input[0].length; j++) {
				quantizedArray[i][j] = input[i][j] * quant_table_luma[i % 8][j % 8];
